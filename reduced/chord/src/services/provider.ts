...
import { getReplicatedStateInternals, serviceDeliveryContext, type ReplicatedStateInternals } from "./state.ts";
...
/** Serves remote service implementations and owns each connected consumer's subscriptions. */
export class RemoteServiceProvider {
	...
	replace<T>(service: Service<T>, implementation: object): void {
		this.#assertActive();
		this.#assertRemotable(service);
		this.#assertAllowed(service.id);
		const registration = this.#registration(service.id, "singleton");
