// Reduced from packages/chord/src/facets/host.ts (906 lines) + loader.ts (6 lines).
// Kept verbatim: FacetLifecycle, LocalKeyedServiceRegistry, HostServiceSlots,
// StagedServiceSpawner, the staging/activation/reload/dispose flow, dependency
// validation and topological ordering.
// Dropped: RemoteServiceSource/serviceSources multi-source machinery (the
// reduced host is local-facets-only — there are no external catalogues, no
// absent-service placeholders, no deferred sources), replacement-shape
// validation (the reduced RemoteServiceProvider has no validateReplacement),
// and the diagnostics plumbing. FacetHost.services is typed RemoteServices in
// the reduced contract, so the kernel exposes a host-facing use/observe/ready/
// dispose facade (local singletons served directly, everything else through the
// loopback binding) instead of the raw RemoteServiceProvider.
...
export class FacetKernel {
	...
	#internalServices: RemoteServiceBindingImpl | undefined;
	#localKeyedServices: LocalKeyedServiceRegistry | undefined;
	#localSingletons = new Map<string, object>();
	#services: (RemoteServices & RemoteServiceTransport) | undefined;
	...
	/** Host-facing consumer surface. The reduced host has no wire-side provider exposure. */
	get provider(): RemoteServices & RemoteServiceTransport {
		if (this.#services === undefined) throw new Error("Facet service provider is not assembled");
	...
	async activate(): Promise<void> {
		...
		try {
			...
			await this.#internalServiceBinding.ready(BACKGROUND_CONTEXT);
		...
		try {
			for (const candidate of candidateOrder) {
				for (const provision of candidate.provisions) {
					...
					if (provision.service.local) {
						...
						this.#localSingletons.set(provision.service.id, provision.implementation);
					} else {
						provision.replace(this.#remoteProvider);
					}
				}
			}
			...
			for (const candidate of candidateOrder) {
				for (const provision of candidate.provisions) {
					...
					else provision.connectRemote(this.#remoteProvider);
				}
			}
		} catch (error) {
			...
			provideMany: <T>(service: Service<T>): ServiceSpawner<T> => {
				...
				provisions.push({
					...
					connectRemote: (provider) =>
						instances.connect((key, implementation) => provider.spawn(service, key, implementation)),
				});
				return instances;
			},
	...
	#createHostServices(): RemoteServices & RemoteServiceTransport {
		return {
			use: <T>(service: Service<T>): T => {
				this.#assertServiceTargetAccess();
				if (service.local) {
					const implementation = this.#localSingletons.get(service.id);
					if (implementation === undefined) throw new Error(`Local service ${service.id} has no provider`);
					return implementation as T;
				}
				return this.#internalServiceBinding.use(service);
			},
			observe: <T>(
				service: Service<T>,
				handler: (service: T, context: Context) => void | Promise<void>,
				...
				this.#assertServiceTargetAccess();
				if (service.local) return this.#localKeyedRegistry.observe(service, handler);
				return this.#internalServiceBinding.observe(service, handler);
			},
			ready: (context: Context) => this.#internalServiceBinding.ready(context),
			dispose: async (context: Context): Promise<void> => {
				this.#localKeyedServices?.dispose();
				await this.#internalServices?.dispose(context);
			},
			invoke: (call, context) => {
				const provider = this.#provider;
				if (provider === undefined) throw new Error("Facet remote service provider is not assembled");
				return provider.invoke(call, context);
			},
			subscribe: async (serviceId, mode, listener) => {
				const provider = this.#provider;
				if (provider === undefined) throw new Error("Facet remote service provider is not assembled");
				return provider.subscribe(serviceId, mode, listener);
			},
		};
	}
	...
	#assembleProviders(): void {
		...
		this.#localSingletons = new Map(
			provisions.flatMap((provision) =>
				provision.kind === "singleton" && provision.service.local
					? [[provision.service.id, provision.implementation] as const]
					: [],
			),
		);
		this.#services = this.#createHostServices();
	...
	get #remoteProvider(): RemoteServiceProvider {
		if (this.#provider === undefined) throw new Error("Facet remote service provider is not assembled");
	...
	get #internalServiceBinding(): RemoteServiceBindingImpl {
	...
	async #disposeServiceBindings(): Promise<unknown[]> {
		const binding = this.#internalServices;
...
function validateFacets(records: readonly FacetShape[]): string[] {
	// The reduced host has no external service sources, so every provider is a facet.
...
export async function disposeLoadedFacets(loaded: readonly LoadedFacets[]): Promise<unknown[]> {
	const results = await Promise.allSettled(loaded.map((entry: LoadedFacets) => entry.dispose()));
