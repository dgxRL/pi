import { BACKGROUND_CONTEXT } from "../../chord/src/index.ts";
import {
	encodeCbor,
	encodeFrame,
	encodeServerMessage,
...
describe("Client service operations", () => {
	...
	test("buffers service updates until the subscription snapshot arrives", async () => {
		...
		const transport = createClientServiceTransport(client, () => serverTarget);
		...
		server.send({
			...
			update: { type: "state", member: "state", sequence: 99, ops: [["s", ["revision"], 99]] },
		});
		await Promise.resolve();
...
describe("Client connection lifecycle", () => {
	...
	test("rejects pending requests and reconnects through a fresh transport", async () => {
		...
		const pending = client.request(serverTarget, { serviceId: "test.session", member: "run", args: [] });
		await first.waitForMessages(2);
		expect(first.messages[1]).toMatchObject({ call: { serviceId: "test.session", member: "run" } });
		first.disconnect();
