// Reduced from packages/telemetry/src/index.ts.
// Explicit, callback-based telemetry contract plus typed schema utilities.
// Dropped: `sensitive`/`cardinality` attribute metadata and `examples` fields.
...
export type TelemetryAttributeDefinition = TelemetryAttributeMetadata &
	(
		...
		| { type: "boolean"; values?: readonly boolean[] }
		| { type: "string[]"; elementValues?: readonly string[] }
		| { type: "number[]"; elementValues?: readonly number[] }
		| { type: "boolean[]"; elementValues?: readonly boolean[] }
	);
...
export type TelemetrySchemaSpanUnion<Schema extends TelemetrySchemaDefinition> = {
	[Name in TelemetrySchemaSpanName<Schema>]: SchemaTelemetrySpan<Schema, Name>;
