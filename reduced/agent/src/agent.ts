// Reduced from packages/agent/src/agent.ts (607 lines) — the stateful Agent
// wrapper around the low-level agent loop.
//
// Dropped relative to the original:
// - `onPayload`/`onResponse`/`transport`/`thinkingBudgets`/`maxRetryDelayMs`
//   option plumbing; `createLoopConfig` forwards only the per-request options
//   the reduced StreamOptions carry ({ apiKey?, sessionId?, signal? }).
// - The prompt-with-images overload (ImageContent is not in the reduced types).
// - The "older compiled consumers may omit options" compat shim: `options`
//   and `options.streamFn` are required; the `getDefaultStreamFn()` fallback
//   in the constructor is gone.
// - The `reasoning` mapping in `createLoopConfig`: the reduced AgentLoopConfig
//   has no reasoning field. `thinkingLevel` stays on the state for future use.
// - The `sessionId` option on `AgentOptions` (the loop config still has the
//   field for per-request forwarding, but nothing populates it here).
